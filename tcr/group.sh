...
for file in cluster_reports/*_cluster_report.tsv; do
    srr=$(echo "$file" | sed -E 's/.*_(SRR[0-9]+)_.*$/\1/')

    if [ $header_written -eq 0 ]; then
      awk -v srr="SRR" -v OFS="\t" -v IFS="\t" 'NR==1 {print srr, $2, $3, $4, $5, $6, $7, $8, $9, $10}' "$file" > "$REPORT"
        # awk -v srr="SRR" 'NR==1 {print srr "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9 "\t" $10}' "$file" > "$REPORT"
        header_written=1
    fi
