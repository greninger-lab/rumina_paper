...
- [File and directory descriptions](#file-and-directory-descriptions)
  ...
  - [`hiv_simulation`](#hiv_simulation)
    ...
    - [File descriptions](#file-descriptions-2)
...
Before aligning, download the gzipped FASTAs for mm9 chromosomes 1-19, X, Y and M from [here](https://hgdownload.soe.ucsc.edu/goldenPath/mm9/chromosomes/). Unzip them, and concatenate them into `../mm9.fa`.
```bash
...
2. Then map the FASTQs to generate the BAMs. You'll need to download `hg38.analysisSet.fa.gz` from [here](https://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/analysisSet/).
```bash
...
6. Finally, run iVar variants to generate SNV data.
```bash
